...
class Maze:
    ...
    def move_turtle(self, x, y):
        self.t.up()
        self.t.setheading(
            self.t.towards(x + self.x_translate, -y + self.y_translate,)
        )
...
def search_from(maze, start_row, start_column):
    """Try each of four directions from this point
    ...
    if (
        ...
        return False
    # 3. We have found an outside edge
...
my_maze = Maze("./maze_data.txt")
my_maze.draw_maze()
my_maze.update_position(my_maze.start_row, my_maze.start_col)
search_from(my_maze, my_maze.start_row, my_maze.start_col)
my_maze.wn.exitonclick()
# my_maze.wn.getcanvas().postscript(file=f"maze.ps")